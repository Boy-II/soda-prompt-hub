...
1. 默认继续使用 LM Studio；只有本地模型不适合当前任务时，才展开创作台左栏的“连接外部模型（可选）”。
2. 填写服务名称、OpenAI-compatible Base URL 和 API Key，点“读取可用模型”；读取失败时也可手工填写真实模型名。
3. 勾选“这个模型可以看图”前先确认服务和模型确实支持视觉输入。选择外部视觉模型时，本地图片会发送到该服务。
4. 保存后 Key 输入框会清空。修改名称、模型名或视觉标记时点“编辑”，Key 留空会保留原密钥。
5. 密钥只保存在 `prompt-library/private/model-connections.json`，接口不回显；远程服务必须是 HTTPS，本机地址才允许 HTTP。
6. 首版只支持 OpenAI-compatible，不把 Claude/Gemini 原生接口伪装成已支持。
