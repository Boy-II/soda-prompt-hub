...
- LoRA 图片卡初次渲染只有 12 个状态选择器、0 个分类复选框；展开一张后才生成该图的 48 个分类/风险控件，避免 72 张一次生成 3,528 个输入控件。
- 设备任务页初次进入时 LoRA 与底模网格均为 0 个子节点，但页签保留 258 / 144 的轻量计数；桌面打开对应页后每页 24 张，390px 下每页 12 张。
- 提示词库 30 条结果按 12 条分页；第二页卡片继续使用全局索引 12、13、14，收藏和备注不会因分页指向错误条目。
- 数据集工作台保持“选择当前筛选结果”为整个筛选集合；卡片改为桌面每页 24、宽度不超过 700px 时每页 12，跨过断点自动回到第一页。
- 桌面 1280×720 与窄屏 390×844 实测均无横向溢出；隔离服务控制台没有 warning/error。

## 2026-09-06：阶段 46 长列表审计

- 1462×711 桌面端：设备 LoRA 258 张卡片约 43,833px，底模 144 张约 24,138px，LoRA 项目 72 张审核卡约 12,801px，提示词库 30 张卡片约 9,644px。
- 684×756 窄屏：设备 LoRA 约 53,784px，底模约 28,196px，LoRA 项目约 17,043px，数据集 72 张约 31,779px，提示词库约 7,685px；均无横向溢出。
- 设备页即使停留在任务子页，也会同时请求并渲染 258 个 LoRA 与 144 个模型；实测隐藏模型卡 402 张、页面 DOM 节点约 17,944 个，182 张图片已经加载，是切换迟缓的主要风险。
- 根因均为前端对完整结果直接 `map(...).join('')`；数据集虽已有分页控件，但 `pageSize=200` 使 72 张真实数据全部落在一页。
- LoRA 图片卡预先为每张图生成 49 个表单控件；72 张共生成 3,528 个输入控件，即使详情默认收起仍占 DOM。应在展开时再生成编辑器。

## 2026-09-06：设备任务页重复清单诊断

- 真实页面显示 18 条“需要处理”，其中大量是已经被更新快照取代的 LoRA/底模清单任务；ComfyUI 图片任务必须继续逐条保留，不能按同类规则自动隐藏。
- 浏览器实际点击最新底模任务的“检查文件完整性”后，服务成功校验 162 个输出且无 console 错误，但按钮在数秒内只设置 `disabled`，没有文案或视觉反馈，因此用户会认为按钮点不动。
- 正式缓存已在 2026-09-06 13:20 左右更新为 258 个 LoRA 与 144 个模型；对应任务仍位于 `inbox` 并显示“等待接收”，证明导入路径没有记录 Mac 已验收状态。
- `import_returned_lora_catalog`、`import_returned_model_catalog` 和 ComfyUI `import-results` 在成功后都没有更新本地任务事实；`list_tasks` 始终按 SMB `inbox` 位置返回 `returned`，这是重复卡片长期堆积的根因。
- 修复边界：不删除任务或输出；用本地验收标记覆盖展示状态，并对旧清单做“只展示最新、其余折叠为被取代”的前端归类。
...
- 当前 `dataset_curation.py` 1791 行、`windows_worker.py` 1531 行、`remote_nodes.py` 1518 行；三者都是单一大型 class 加一组底部 helper，适合先按纯函数/适配器边界提取，而不是拆公开 class 接口。
- `web.py` 1167 行，仍包含主 CSS、导航 HTML 和主 JavaScript；各功能页已经分别位于 `creative_web.py`、`workspace_web.py`、`remote_web.py` 等模块，因此静态资源拆分应先从主壳层开始，避免重做所有页面。
- 主 SQLite 由 `database.py`、`creative.py`、`background_jobs.py` 分散执行 `CREATE TABLE IF NOT EXISTS` 和局部 `ALTER TABLE`；没有统一 `PRAGMA user_version` 或 schema 迁移台账。
- 原审计称“390px 顶部九个入口依赖横向滚动”已经过期：当前 CSS 在 700px 以下把入口改为两列网格。现存体验问题应重新验证为导航占屏高度、入口分组与当前页识别，而不是横向溢出。
- GitNexus skill 已读取，但本会话没有可调用的 GitNexus 图工具；后续用 `rg` 静态符号、调用搜索和测试影响范围替代，并保持接口优先、小步拆分。
- 改动前本机页面标题为 `Soda Prompt Hub`，桌面首页主导航含 9 个功能入口和 1 个标签语言切换，页面能读取 33,633 条资料、6 个来源，未出现错误覆盖层。手机尺寸与数据集页仍需继续做改动前证据。
- 四个风险模块的定向基线为：`model_connections.py` 60%、`wd14.py` 50%、`mcp_server.py` 61%、`web_capture.py` 65%；29 项现有测试全过。缺口主要在损坏配置、模型/标签文件异常、MCP 工具实际调用和网络响应错误分支。
- `model_connections` 已有 API Key 权限、掩码、保留旧密钥、URL 安全和本地/外部请求形状测试，但缺配置 JSON 损坏、未知 ID/provider、删除不存在连接及模型列表响应失败测试。
- `wd14` 现有测试只覆盖图片预处理、标签加载/排序和 auto CPU；构造器阈值、缺文件、空/坏 CSV、CoreML 缺失、坏图、模型输出形状均未覆盖。
- `mcp_server` 只断言工具名称，没有实际调用八个工具；同时服务器版本仍硬编码为 `0.1.3`，与已发布 `1.0.0` 不一致，应作为产品称呼/版本清理的一部分修正。
- 当前 MCPServer 提供公开异步 `call_tool(name, arguments)`，返回 `CallToolResult.structured_content`；可以在测试中直接执行工具，不需要启动 stdio 子进程或依赖私有 manager。
- 1280px 桌面真实页面原先仍因十个导航项固定最小宽度产生内部横向滚动；在 901–1400px 收紧按钮后，导航 `clientWidth` 与 `scrollWidth` 均为 1007px，滚动条消失。
- 390×844 手机真实页面中，主导航默认只保留品牌和“当前页面 / 打开菜单”；展开后九个入口与标签切换均可见，点击“数据集”后自动收起并更新当前页。页面 `scrollWidth=innerWidth=390`。
- 正式资料库的数据集页优先显示“林悔儿 · v012 旧训练数据集”的继续入口与 72 张状态，导入区改称“导入另一个文件夹”；隔离空资料库中继续入口隐藏，首要入口保持“01 · 导入素材”。两种状态均无 console warning/error。
- 应用内浏览器完成桌面与正式数据交互验收，但不提供视口调整；精确 390px 和空资料库状态使用已验证的 Playwright CLI 补充。仅点击导航和读取 DOM，没有提交表单或改变正式数据。
...
- 当前 `codex/post-v1-maintainability-ux` 仍有两个未进入 `main` 的提交：`894bc99`（Windows Worker 独立发布包）与 `8d6eba2`（公开用户和工作流教程）。
- 工作区另有 12 个阶段 45–46 的未提交文件，共约 320 行新增、55 行删除；其中 `remote_web.py` 与 PR 6 有重叠，需在干净分支逐段整合。
- 远程当前没有开放 PR 或 issue；本轮目标是创建一个收口 PR，不扩展训练、视频或 Windows VLM/Embedding 范围。
- `remote_web.py` 的冲突集中在模型端点子页与 LoRA/底模延迟加载；兼容解法是初始加载端点和两类清单数量，真正的 LoRA/底模条目只在打开对应子页时读取。
- `workspace_web.py` 的冲突集中在 PR 6 新增的文件夹浏览/ZIP 导入/模型打标状态与阶段 46 的分页状态；二者可以直接合并，卡片继续显示实际打标模型名称，同时桌面每页 24 张、窄屏每页 12 张。
- 隔离浏览器证明合并后模型接入仍要求先拉取模型：空列表时“保存端点与勾选”禁用，页面没有重新出现手动模型输入；手机 390px 的四个设备子页和数据集导入区没有横向溢出。
