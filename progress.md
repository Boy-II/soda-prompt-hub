...
## 2026-09-07：阶段 49 候选版人工检查与任务页收口

- 根据正式资料页截图复核 20 条任务占满页面的问题；确认这些记录全部为 Windows 共享目录离线时只能读取到的 Mac 本地事实副本，不应误写成“等待发送”，也不能在未连接共享目录时直接删除。
- 首页现在永久保留“第一次使用？查看 5 步指南”，不再随着推荐资料库安装完成而消失。
- 设备页默认只显示最近 6 条当前任务，其余 14 条按需展开；展开后可以再次收起，历史记录继续独立折叠。
- 离线状态的顶部摘要改为“0 条需要你处理 · 20 条等待连接 Windows 后核对”，页签显示“20 待确认”，避免用户误以为必须逐条确认。
- 没有取消、删除或修改现有 20 条任务；真正处于 Windows 共享目录 `queued` 的任务，仍只在连接恢复后提供一次确认的批量取消入口。
- 本机真实页面完成首页指南、任务 6→20→6 展开收起和 390×844 手机宽度验收，控制台 0 warning/error；浏览器最后保留在已展开的首次使用指南。
- 最新工程门通过：132 个文件 Ruff format、Ruff lint、`ty check src/`、`uv lock --check`、301/301 pytest、81.96% coverage、Python wheel/sdist、全部 Mac `.command` 语法与 `git diff --check` 均正常。
- Windows Worker 候选 ZIP 重新构建并通过包内 manifest、隐私边界和压缩完整性测试；最新 SHA-256 为 `9731bd15530775d88cccabc8e56e66ceb869bb7db59193f9ede7e9a5ed781228`。
- 服务继续运行在 `http://127.0.0.1:8765/`。Windows 当前离线，因此本轮没有做真实 Worker/ComfyUI 联机验收；未 commit、未 push、未创建 PR、tag 或 Release。
...
- 2026-09-07：用户授权开始阶段 48 P0 开发。范围固定为 Mac 首次安装与通用启动、现有四个推荐提示词库的一键准备、设备页自动诊断；AnimaDex importer、DMG、自动训练和 Windows 新协议不在本轮。
- 审计确认批量资料拉取与重建索引的后端能力已经存在，首次安装和设备诊断主要缺公开入口与前端状态闭环；当前设备名称通用化改动继续保留在同一分支，未 commit、未 push。
- 阶段 48 的 Mac 首次安装器和三个通用启动脚本已完成首轮实现。隔离安装成功准备 Python 3.12、37 个运行依赖、程序命令与独立资料目录；四个脚本均通过 `zsh -n`。首次隔离测试发现 worktree 的 `.git` 文件会被复制，已将 `.git`、测试、开发缓存和规划文件加入排除清单，待二次复验。
- 首页已增加只在推荐来源缺失时出现的轻量资料准备卡，复用现有后台任务一次拉取全部缺失源并重建索引；显示来源许可证、视觉库体积提示、进度与失败摘要，并支持按缺失来源组合暂时跳过。
- 设备页已提取统一诊断渲染逻辑：打开页面自动检查已保存设备，保存后立即复查，手动检查和创建任务目录继续共用相同中文结果；未保存的默认卡仍显示“尚未配置”，诊断失败不弹窗。
- 首轮前端 Ruff 检查通过；JavaScript 语法检查第一次误用系统 Python而无法导入包，尚未实际进入 Node 检查，已记录并改用项目环境重跑。
- 隔离桌面浏览器确认首页能显示 4 个缺失来源、实际许可证与视觉库提醒，页面标题正确且 console 0 warning/error。首次点击“暂时跳过”时发现首页说明 `aside` 吸顶后遮挡按钮，已取消该说明栏的吸顶行为，等待同路径复测。
- 同一路径复测后，“暂时跳过”可点击并在刷新后保持收起；安装按钮会先显示四个来源、许可证和视觉库体积确认框。390×844 下页面宽度为 390/390，无横向溢出，console 0 warning/error；当四个来源都不再缺失时，首页准备卡自动隐藏。
- 设备页真实交互确认：打开已保存节点会自动诊断为“共享目录未挂载”；保存一个存在但尚未准备的测试挂载目录后，无需再点检查，状态自动更新为“需要创建任务文件夹”。整个流程 console 0 warning/error。
- 首轮完整工程门通过：118 文件 Ruff format、Ruff lint、ty、`uv lock --check`、288 项 pytest（81.92% coverage）、8 段页面 JavaScript、4 个 Mac 脚本语法与 `git diff --check` 全部通过。
- 收尾审计补强首次下载中断后的重试：仅当目标目录原本不存在时，失败或超时会清理本次新建的半成品；原有非空目录仍拒绝覆盖。新增回归覆盖该边界，完整质量门需在此修改后重新执行。
- 第二轮工程门中 289 项 pytest 与 8 段 JavaScript 通过，但 Ruff 准确发现新增测试残留未使用的 `tmp_path` 参数，因此本轮不能视为通过；已删除该参数，下一轮使用失败即停止的方式从头复验。
- 阶段 48 最终质量门从头通过：118 个文件 Ruff format、Ruff lint、ty、`uv lock --check`、289 项 pytest（81.92% coverage）、8 段页面 JavaScript、4 个 Mac `.command` 语法与 `git diff --check` 全部正常，输出标记为 `FINAL_GATE=PASS`。
- 隔离首次安装再次验证成功：Python 3.12、37 个运行依赖与独立数据库均正常；安装目录不包含 `.git`、tests、缓存或规划文件，安装后的服务可在独立端口健康启动。
- 桌面 1280×800 与手机 390×844 验收完成：首页资料准备卡、跳过状态、安装确认框、来源齐全后自动隐藏均正常，手机无横向溢出，console 0 warning/error；设备页打开和保存后均会自动给出准确中文诊断。
- 阶段 48 标记为 `complete / local`。P0 已完成但尚未 commit、push 或创建 PR；当前分支仍保留此前设备名称通用化改动，未做回退或清理。
- 完成状态写回任务书后再次执行新鲜质量门，结果仍为 289/289 pytest、81.92% coverage、8 个 JavaScript 区块、4 个 Mac `.command` 与全部静态检查通过，最终标记 `FINAL_GATE=PASS`。
- 2026-09-07：用户授权开始阶段 49 P1 正式版本体系开发。范围固定为版本显示、数据迁移版本汇总、Worker 兼容诊断、安全更新与发布清单；不执行正式发版，不 commit、push 或创建 PR。
- 初步审计确认软件版本仍在两处硬编码，数据库迁移和 Worker 协议已有可复用基础；更新器必须只替换程序目录并保留个人资料，提示词资料源与模型更新继续独立。
- 用户补充 P1 必须同时完成文档重新分层、Windows Worker 正式发行和正式版本体系；本轮将生成可验证的本地发行包，但 commit、push 与 GitHub Release 继续等待完成后的单独确认。
- 完成首轮结构审计：Worker 已有独立脚本、示例配置和离线 README，但没有程序版本、发行 manifest、包校验与升级入口；现有数据库迁移台账和 bridge v2 协议可以直接复用。
- P1 实现方案确定：Mac 使用包元数据单一版本源；系统版本 API 只读迁移台账；Worker 使用独立 RELEASE 文件并由白名单构建器生成 SHA-256 清单；协议不兼容与仅版本落后在设备页分开显示。
- 前端接入点确认：首页既有状态区增加程序/数据版本即可，不新增页面；设备诊断将从单一 `worker_ready` 扩展为普通用户可理解的版本与协议状态。
- 版本接口首轮定向检查未通过：Ruff 指出 API import 顺序和 `Path` 仅用于类型；接口测试则发现实际主库还包含 `danbooru_tags=1`。已按真实迁移台账修正，不改变数据库内容。
- Worker 版本组首轮检查发现 9 项新增代码规范/类型问题和 1 个由测试漏导入导致的失败；均未进入个人数据路径，已按项目既有规则修正。
- 第二轮 31 项行为测试与 ty 已通过，Ruff 只剩同一文案中的一处全角逗号；已拆成三个短句后继续。
- 版本/Worker 代码进入实现：软件候选版本改为 `1.1.0.dev1`，新增只读系统版本信息、Worker 协议兼容分类和 Worker 状态/结果版本字段；下一步构建正式 Worker 白名单发行包。
- Worker ZIP 首轮 32 项行为测试通过；Ruff 仅阻止新增构建脚本的包/异常/输出写法，已用脚本包标记、领域异常和 stdout 流修正，没有放宽 lint。
- 2026-09-07：继续阶段 49。已确认 Mac 安全更新器、首次安装器的发行校验增强与 `tests/test_mac_release_scripts.py` 已写入但尚未验证；先执行定向格式、语法与回滚测试，再进入前端、文档和发行包收口。两个新增 Mac `.command` 当前缺可执行位，待验证后补齐。
- Mac 更新器定向验证首轮 2 项失败：更新器优先找到 Homebrew 的真实 `uv`，没有进入测试 fake `uv`；证据由独立命令解析复现。新增显式 `PROMPT_HUB_UV_BIN` 维护覆盖并补全测试项目元数据后，3 项更新器测试、Ruff 与全部 Mac 脚本语法通过；两个新增 `.command` 已补可执行位。
- 首页已接入程序版本、发布通道和数据结构摘要；设备卡新增独立 Worker 版本/协议提示，协议不兼容不再伪装成普通共享目录状态。25 项接口与页面定向测试、8 段页面 JavaScript 语法通过。
- 公开文档已初步分层为 `docs/QUICK_START.md`、`WORKFLOWS.md`、`MAC_GUIDE.md`、`WINDOWS_WORKER.md`、`ARCHITECTURE.md`、`TROUBLESHOOTING.md` 和 `RELEASES.md`；README 缩为产品入口，四份旧根目录指南保留为兼容导航。包内 Worker 说明增加发行包校验、首次配置和安全升级顺序。
- 文档链接与分层回归已加入；人工验收清单清除了作者专属数据数量和不存在的备份脚本，`v1.0.0` 审计/验收报告明确标为历史文档。
- 隔离浏览器完成桌面与 390px 手机验收：首页版本与数据结构可见，旧 Worker 建议更新与协议不兼容两条路径均完成点击/刷新验证；两种视口无横向溢出，console 0 warning/error。真实验收中发现的“协议不兼容却提示等待自检”已修复并复测。
