...
<style>
  ...
  .discovery-model-actions { display: grid; gap: 8px; margin-top: 14px; }
  .discovery-model-actions .visual-action { border-color: rgba(236,232,220,.45); color: var(--paper); padding: 10px 12px; }
  .discovery-model-actions .visual-action.primary { border-color: var(--acid); background: var(--acid); color: var(--ink); }
  .discovery-custom-form { display: grid; gap: 8px; margin-top: 12px; padding-top: 12px; border-top: 1px solid rgba(236,232,220,.22); }
  .discovery-custom-form label { display: grid; gap: 4px; color: #b9ae9f; font: 800 8px/1 monospace; letter-spacing: .06em; }
  .discovery-custom-form input { min-width: 0; border: 1px solid rgba(236,232,220,.35); background: rgba(236,232,220,.08); color: var(--paper); padding: 9px 10px; font: 12px/1.4 monospace; }
  .discovery-custom-form .visual-action { justify-self: start; }
  .discovery-custom-hint { margin: 0; color: #8f8e84; font-size: 10px; line-height: 1.6; }
  ...
  .discovery-index .visual-job span { color: #aaa99f; font-size: 10px; }
...
<section class="discovery-page" id="discoveryPage" hidden>
  <section class="discovery-hero">
    ...
    <aside class="discovery-index"><span class="section-label">图片检索是否可用</span><strong id="discoveryIndexTitle">正在检查</strong><p id="discoveryIndexMessage">图片检索需要本机视觉模型和已经建立的图片索引。</p><div class="discovery-index-list" id="discoveryIndexList"></div><div class="discovery-model-actions" id="discoveryModelActions" hidden><button class="visual-action primary" id="visualDownloadButton" type="button">拉取视觉模型</button><button class="visual-action" id="visualCustomToggle" type="button">使用自定义视觉模型</button><button class="visual-action" id="visualRestoreBundledButton" type="button" hidden>切回官方视觉模型</button></div><div class="discovery-custom-form" id="visualCustomForm" hidden><label>模型文件路径<input id="visualCustomPath" placeholder="/Users/you/models/vision_model.onnx"></label><label>模型标识<input id="visualCustomModelId" placeholder="my-clip-vit" required></label><label>模型版本<input id="visualCustomRevision" placeholder="v1" required></label><label>输出维度<input id="visualCustomDimension" type="number" min="1" max="8192" value="512"></label><label>输入边长<input id="visualCustomInputSize" type="number" min="16" max="4096" value="224"></label><p class="discovery-custom-hint">模型文件按只读来源对待，不会被移动或改写。启用前会用 onnxruntime 实跑一张合成图，输出维度与声明不符将拒绝启用。</p><button class="visual-action primary" id="visualCustomEnable" type="button">校验并启用</button></div><div class="visual-job" id="visualDownloadJob" hidden><strong id="visualDownloadJobTitle">视觉模型下载任务</strong><progress id="visualDownloadJobProgress" value="0" max="1"></progress><span id="visualDownloadJobMessage">等待开始</span><div class="visual-actions"><button class="visual-action" id="visualDownloadPause" type="button">取消</button><button class="visual-action" id="visualDownloadRetry" type="button" hidden>重试</button></div></div></aside>
  </section>
...
<script>
(() => {
  const state={indexes:[],payload:null,visualStatus:null,visualFile:null,visualJob:null,downloadJob:null,mode:'text'};
  ...
  async function loadIndexes() { state.visualStatus=await api('/api/visual-index/status'); state.indexes=state.visualStatus.indexes||[]; const model=state.visualStatus.model||{}, current=state.visualStatus.current_index, counts=state.visualStatus.type_counts||{}; const mode=state.visualStatus.mode||'bundled', custom=state.visualStatus.custom||null; $('#visualBuildButton').disabled=!model.available; $('#visualDownloadButton').hidden=model.available||mode==='custom'; $('#visualRestoreBundledButton').hidden=mode!=='custom'; $('#visualCustomToggle').textContent=mode==='custom'?'更换自定义视觉模型':'使用自定义视觉模型'; $('#discoveryModelActions').hidden=false; renderVisualDownloadJob(state.visualStatus.download_job||null); const modelLine=mode==='custom'&&custom?`当前模型：自定义 · ${escapeHtml(custom.model_id)}@${escapeHtml(custom.model_revision)} · ${custom.dimension} 维`:`当前模型：官方 · ${escapeHtml(model.model_id||'未知')}@${escapeHtml(String(model.model_revision||'').slice(0,10))} · ${model.dimension} 维`; const indexLine=current?`当前索引：${formatNumber(current.item_count)} 张，属于 ${escapeHtml(current.model_id||'')}`:state.indexes.length?`现有索引属于其他模型，需要重新建立索引`:`还没有建立图片索引`; const metaLines=[modelLine,indexLine].map(line=>`<div class="discovery-index-item">${line}</div>`).join(''); if(!model.available) { $('#discoveryIndexTitle').textContent='图片检索暂不可用'; $('#discoveryIndexMessage').textContent=mode==='custom'?'自定义视觉模型文件不存在，目前仍可使用文字检索。':'本机还没有安装所需的视觉模型，目前仍可使用文字检索。'; $('#discoveryIndexList').innerHTML=metaLines; $('#visualIndexNote').textContent=mode==='custom'?'自定义模型文件不存在：可以更换模型，或切回官方模型再拉取。':'点击“拉取视觉模型”即可安装，下载进度会在这里显示。'; return; } if(!current) { $('#discoveryIndexTitle').textContent='视觉模型已安装'; $('#discoveryIndexMessage').textContent='还没有建立图片索引。点击“建立 / 更新本地索引”即可开始。'; $('#discoveryIndexList').innerHTML=metaLines; $('#visualIndexNote').textContent='首次建立可以暂停；继续时会跳过已经完成的图片。'; return; } $('#discoveryIndexTitle').textContent=`已经索引 ${formatNumber(current.item_count)} 张图片`; $('#discoveryIndexMessage').textContent='索引在 Mac 本地生成，图片不会上传。'; $('#discoveryIndexList').innerHTML=metaLines+Object.entries(counts).map(([key,value])=>`<div class="discovery-index-item">${escapeHtml(assetTypeLabels[key]||'其他图片')} · ${formatNumber(value)} 张</div>`).join(''); $('#visualIndexNote').textContent=`再次更新时，只处理新增或已经变化的图片。`; }
  ...
  function renderVisualDownloadJob(job) { state.downloadJob=job||null; if(!job){$('#visualDownloadJob').hidden=true;return;} const active=['queued','running'].includes(job.status), failed=job.status==='failed', canceled=job.status==='canceled'; $('#visualDownloadJob').hidden=false; $('#visualDownloadJobTitle').textContent=job.status==='completed'?'视觉模型已安装':failed?'视觉模型下载失败':canceled?'视觉模型下载已取消':active?'正在拉取视觉模型':'视觉模型下载任务'; $('#visualDownloadJobProgress').max=Math.max(job.progress_total||1,1); $('#visualDownloadJobProgress').value=job.progress_current||0; $('#visualDownloadJobMessage').textContent=job.status==='completed'?'模型校验通过，可以建立图片索引。':job.progress_message||job.error||'等待处理'; $('#visualDownloadPause').hidden=!active; $('#visualDownloadRetry').hidden=!(failed||canceled); }
  async function pollVisualDownloadJob(job) { renderVisualDownloadJob(job); while(['queued','running'].includes(job.status)){await new Promise(resolve=>setTimeout(resolve,800));job=await api(`/api/jobs/${encodeURIComponent(job.job_id)}`);renderVisualDownloadJob(job);} if(job.status==='completed'){await loadIndexes(); $('#discoveryStatus').textContent='视觉模型已安装并校验通过，可以建立图片索引。';} else if(job.status==='failed'){ $('#discoveryStatus').textContent=`模型下载失败：${job.error||'未知错误'}。可以点击重试，已下载的部分会续传。`; } return job; }
  async function startVisualDownload() { $('#visualDownloadButton').disabled=true; try { const result=await api('/api/visual-index/model/download',{method:'POST'}); await pollVisualDownloadJob(result.job); } finally { $('#visualDownloadButton').disabled=false; } }
  async function cancelVisualDownload() { if(!state.downloadJob)return; const job=await api(`/api/jobs/${encodeURIComponent(state.downloadJob.job_id)}/cancel`,{method:'POST'}); renderVisualDownloadJob(job); $('#discoveryStatus').textContent='已取消下载。已经下载的部分会保留，重试时断点续传。'; }
  async function retryVisualDownload() { if(!state.downloadJob)return; const job=await api(`/api/jobs/${encodeURIComponent(state.downloadJob.job_id)}/retry`,{method:'POST'}); await pollVisualDownloadJob(job); }
  function toggleVisualCustomForm() { const form=$('#visualCustomForm'); form.hidden=!form.hidden; }
  async function enableVisualCustom() { const path=$('#visualCustomPath').value.trim(), modelId=$('#visualCustomModelId').value.trim(), revision=$('#visualCustomRevision').value.trim(); const dimension=Number($('#visualCustomDimension').value), inputSize=Number($('#visualCustomInputSize').value); if(!path||!modelId||!revision){$('#discoveryStatus').textContent='请填写模型文件路径、模型标识和模型版本。';return;} $('#visualCustomEnable').disabled=true; $('#discoveryStatus').textContent='正在用 onnxruntime 实跑验证模型，请稍候……'; try { const payload=await api('/api/visual-index/model/custom',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({path,model_id:modelId,model_revision:revision,dimension,input_size:inputSize})}); $('#visualCustomForm').hidden=true; await loadIndexes(); $('#discoveryStatus').textContent='自定义视觉模型已启用。模型已变化，需要重新建立索引。'; } catch(error) { $('#discoveryStatus').textContent=error.message; } finally { $('#visualCustomEnable').disabled=false; } }
  async function restoreBundledVisualModel() { try { const payload=await api('/api/visual-index/model/custom',{method:'DELETE'}); $('#visualCustomForm').hidden=true; await loadIndexes(); $('#discoveryStatus').textContent='已切回官方视觉模型。模型已变化，需要重新建立索引。'; } catch(error) { $('#discoveryStatus').textContent=error.message; } }
  ...
  $('#visualDownloadButton').addEventListener('click',()=>startVisualDownload().catch(error=>$('#discoveryStatus').textContent=error.message));
  $('#visualDownloadPause').addEventListener('click',()=>cancelVisualDownload().catch(error=>$('#discoveryStatus').textContent=error.message));
  $('#visualDownloadRetry').addEventListener('click',()=>retryVisualDownload().catch(error=>$('#discoveryStatus').textContent=error.message));
  $('#visualCustomToggle').addEventListener('click',toggleVisualCustomForm);
  $('#visualCustomEnable').addEventListener('click',()=>enableVisualCustom().catch(error=>$('#discoveryStatus').textContent=error.message));
  $('#visualRestoreBundledButton').addEventListener('click',()=>restoreBundledVisualModel().catch(error=>$('#discoveryStatus').textContent=error.message));
